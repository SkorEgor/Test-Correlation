...
def correlation_variation(data_processing: DataAndProcessing):
    ...
    # for correlation_width in range(10, 41, 5):
    #     for threshold in range(-40, 81, 5):

    for correlation_width in range(20, 41, 5):
        for threshold in range(70, 81, 5):
            # (3) ОБРАБОТКА
            # Чистим от прошлых данных
            data_processing.clear_data_processing()

            # (1) КОРРЕЛЯЦИЯ
            data_processing.correlate(correlation_width)
            # Значение порога корреляции
            data_processing.correlation_threshold = threshold / 100
            # Участки ниже порога. Расширение участков корреляции, с учетом тренда
            data_processing.correlation_extension()
            # Интервалы линий поглощение после корреляции (логический массив)
            data_processing.find_intervals_after_correlation()
            # Находим
            data_processing.find_point_after_correlation()  # Точки линий поглощения, от корреляции
            ...
            for f in data_processing.point_absorption_after_correlation["frequency"]:
...
# Проводит одиночный текст параметров корреляции
...
# Проводит несколько тестов с мультипоточностью
def test_multiprocessing(data_processing: DataAndProcessing):
    task = []
    for correlation_width in range(20, 41, 5):
        for threshold in range(70, 81, 5):
            task.append((data_processing, correlation_width, threshold))

    with Pool(3) as p:
        frames = p.starmap(test_correlation, task)
...
if __name__ == '__main__':
    ...
    file_name_without_gas = "../Detector - Correlation/25empty.csv"
    # Путь к файлу с газом
    file_name_with_gas = "../Detector - Correlation/25DMSO.csv"
    # Порог
    ...
    result = test_multiprocessing(data_signals)
    ...
    with pd.ExcelWriter('out_statistics/statistics_data_cor2.xlsx', mode='w') as writer:
